...
void MainWindow::on_pushButton_5_clicked() {
  ...
  update_parametrs(ui->type_avtopilot->currentText(),
                   ui->Match_model->currentText(), arrBool,
                   ui->lineEdit_12->text().toFloat(), ui->lineEdit_11->text());
}
//кнопка создания маршрутной точки
void MainWindow::on_pushButton_4_clicked() {
  add_waypoint(true, true, true, true, ui->lineEdit_3->text().toFloat(),
...
void MainWindow::on_pushButton_8_clicked() {
  add_maneuver(ui->number_point->currentIndex(),
               ui->types_of_maneuvres->currentText());
}
...
void MainWindow::on_pushButton_15_clicked() { set_start_position(); }
//задать параметры как конечные значения для моделирования
void MainWindow::on_pushButton_16_clicked() { set_finish_position(); }
// считать из файла
...
void MainWindow::start() {
 ...
 QSerialPort::Baud9600; serialPort.setBaudRate(serialPortBaudRate);

     if (!serialPort.open(QIODevice::ReadOnly)) {
         //      standardOutput << QObject::tr("Failed to open port %1, error:
 %2")
         //                        .arg(serialPortName)
...
void MainWindow::exit() {
  ...
  // сохранение (информация/значения датчиков) и (информация/ошибки) в файлы
  // программы заданные по умолчанию
}
...
void MainWindow::update_parametrs(QString typeaudopilot, QString typemodel,
  ...
  if (typemodel == "Квадрокоптер") {
    WMathmodel = new mathmodel_quatro({0, 0}, 0);
  }
}
// смена вида управления
void MainWindow::view_control(int number_view) {}
// добавление точки маршрута
void MainWindow::add_waypoint(bool TVPS, bool TVV, bool GSK, bool SSK, float X1,
                              float X2, float X3) {
...
} // добавление маневра в список маневров с порядковым номером  number_point и
  // типом маневра   maneuver

std::vector<float> MainWindow::modeling_waypoint(float t1, float t2,
                                                 std::vector<float> V1) {}

void MainWindow::readFile(QString file_name, std::vector<QString> arr) {}

//запись в файл массив float
void MainWindow::writeFile(QString file_name, std::vector<float> arr) {}
