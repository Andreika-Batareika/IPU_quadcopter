...
mathmodel_quatro::mathmodel_quatro(std::vector<float> X_come, float come_mass)
    : mathmodel(X_come) {
  a = 0.4;
  h1 = 0.4;
  h = 0.15;
  ...
  X = X_come; // X Y Z Vx Vy Vz speed_heating speed_pitch speed_roll  heating
              // pitch roll
...
std::vector<float> mathmodel_quatro::rightpart(
    float t) // правые части диференциальных уравнений модели
{
  ...
  X[4];
  Uy * 1 / mass *u1;
  std::vector<float> F_out(8 * 8);
  return F_out;
}

//вычисления   ..
std::vector<float> mathmodel_quatro::U_signal() {
