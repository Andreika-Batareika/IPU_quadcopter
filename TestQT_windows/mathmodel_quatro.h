...
public:
  ...
  float b;
  float l;
  mathmodel_quatro(std::vector<float> X_come, float mass);
  void DU(float t);
  // void  B(float t);
  // void  U(float t);// функция выи
  std::vector<float>
  rightpart(float t); // правые части диференциальных уравнений модели
  std::vector<float> U_signal();
  std::vector<float> X; // X Y Z Vx Vy Vz speed_heating speed_pitch speed_roll
                        // heating pitch roll
